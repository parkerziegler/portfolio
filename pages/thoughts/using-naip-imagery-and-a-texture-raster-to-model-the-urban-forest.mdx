...
![A composited NAIP image of the city of Portland, OR in 2014 displayed in true color (R = Band 1, G = Band 2, B = Band 3).](/blogs/naip-imagery-urban-forest/portland-rgb.png)

![A composited NAIP image of the city of Portland, OR in 2014 displayed in color-infrared (R = Band 4, G = Band 3, B = Band 2).](/blogs/naip-imagery-urban-forest/portland-infrared.png)
...
![The NDVI of the NAIP composite displayed using a diverging color ramp. You can use the Image Analysis toolbar to automatically generate an NDVI from an input raster or manually calculate it using Raster Calculator.](/blogs/naip-imagery-urban-forest/portland-ndvi.png)

![The NDVI of the NAIP composite reclassified using a threshold. Remember that this layer gives a good indication of all vegetated surfaces, but does not distinguish trees from grass.](/blogs/naip-imagery-urban-forest/portland-ndvi-composite.png)
...
![The ‘texture’ raster of the NAIP image. Notice how the Willamette River (center) appears very deep blue (low value, smooth surface) while Forest Park (left) appears a tan brown (higher value, rougher surface).](/blogs/naip-imagery-urban-forest/portland-texture-raster.png)

![A zoomed in view of the ‘texture’ raster and the original image. Notice how flat, grassy surfaces appear blue (low values), while the canopy appears tan brown (higher values).](/blogs/naip-imagery-urban-forest/portland-texture-raster-zoom.png)
...
![The output canopy layer from the NAIP imagery after post-classification smoothing and masking. Notice there are still a few holes in canopied areas of Forest Park – those pesky shadows!](/blogs/naip-imagery-urban-forest/portland-canopy.png)

![The output canopy layer overlaid on the original image (left) and the original image (right) for comparison. The classification lines up nicely with the canopy, although it does tend to overestimate crown boundaries in certain places. It also still suffers from some difficulties distinguishing shadows.](/blogs/naip-imagery-urban-forest/portland-canopy-zoom.png)
